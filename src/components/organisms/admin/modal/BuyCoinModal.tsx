...
export default function BuyCoinModal({ isOpen, mode, onClose, refetch }: IBuyCoinModalProps) {
      ...
      if (response != null && 'status' in response && response.status) {
        if ('data' in response && response.data) {
          ...
          setShowCompleteModalData({
            ...
            bankAccount: response.data.rcpmny_dpstr,
            bankAmount: response.data.rcpmny_am,
          });
          refetch?.();
          sideBarRefetch?.();
          onClose();
  ...
  const modeText = mode === 'payment' ? '구매' : '판매';
    ...
    <>
      <Modal isOpen={isOpen} onClose={handleClose} width={'500px'}>
        ...
        <div className={'flex flex-col gap-[30px] pt-2 px-[10px]'}>
          ...
          <div>
            <div className={'text-gray-40 font-pre-15-r-130 py-2'}>{`${modeText} 수량`}</div>
            <div className={'flex justify-between items-center gap-2'}>
              <input
          ...
          <div className={'flex items-center justify-end gap-2'}>
            ...
            <button
              className={`flex-auto  h-[48px] text-gray-100 font-pre-16-m-130 ${
                mode === 'payment' ? 'bg-orange-orange50' : 'bg-red-50'

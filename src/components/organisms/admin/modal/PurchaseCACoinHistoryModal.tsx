...
import type { IPurchaseCACoinHistoryModalProps } from './PurchaseCACoinHistoryModal.types';

import IconLine24Close from '@/components/atoms/icons/icon-line/Close';
import Modal from '@/components/atoms/modals/Modal';
import { useFetch } from '@/hooks';
import { adminPurchaseManagerHistoryService } from '@/services/admin/coin/adminPurchase';
import { convertDealStatus, convertBank, formatPhoneNumber } from '@/utils/covert';

export default function PurchaseCACoinHistoryModal(props: IPurchaseCACoinHistoryModalProps) {
  const { coinHistoryModalTableData, isOpen, onClose } = props;

  const fetchCoinHistory = useCallback(
    () =>
      adminPurchaseManagerHistoryService({
        mngr_id: coinHistoryModalTableData.managerId,
        page: 1,
    ...
    <Modal isOpen={isOpen} onClose={handleClose} width={'960px'} height={'700px'}>
      <div className={'flex flex-col gap-[30px] px-2.5'}>
        <div className={'flex items-center justify-between gap-2 pt-1 pb-3'}>
          <div className={'text-gray-0 font-pre-20-m-130'}>코인 구매 이력</div>
        ...
        <div className={'flex flex-row gap-6'}>
          <div className={'w-[220px] flex flex-col gap-[2px]'}>
            <span className={'w-fit p-1 text-purple-fmg50 font-pre-11-r-113 border border-gray-80 rounded-[4px]'}>
              CA 회원
            </span>
            <div className={'font-suit-24-b-130 text-gray-0'}>{coinHistoryModalTableData.partnerName}</div>
          </div>
            ...
            <div className={'h-[120px] flex flex-col justify-between p-5 border rounded-2xl'}>
              ...
              <div className={'flex flex-row justify-between items-center'}>
                ...
                <div className={'font-pre-16-r-130 text-gray-0'}>
                  {formatPhoneNumber(coinHistoryModalTableData.phoneNumber)}
                </div>
              </div>

              <div className={'flex flex-row justify-between items-center'}>
                <div className={'text-gray-40 font-pre-14-m-130'}>사이트주소</div>
                <div className={'font-pre-16-r-130 text-gray-0'}>{coinHistoryModalTableData.siteUrl}</div>
              </div>
            </div>
          </div>
          ...
          <table className={'w-full border-collapse'}>
            <thead className={'sticky top-0 z-10'}>
              <tr className={'h-8 bg-purple-fmg95'}>
                ...
                <th className={'text-gray-20 font-pre-13-m-130 border border-gray-80'}>구매완료수량</th>
            ...
            <tbody>
              {coinPurchaseHistoryData?.map((item, index) => (
                <tr key={index} className={'h-10 text-center'}>
                  ...
                  <td className={'text-gray-20 font-pre-13-r-130 border border-gray-80 text-end py-1 px-2.5'}>
                    {item.bonusCount?.toLocaleString('ko-KR') || 0}
                  </td>
                  <td className={'text-gray-20 font-pre-13-r-130 border border-gray-80 text-end py-1 px-2.5'}>

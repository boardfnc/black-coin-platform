'use client';

import type { ChangeEvent } from 'react';
import { useState } from 'react';

import { IconLine24SquareInfo } from '@/components/atoms/icons/icon-line';
import { Image } from '@/components/atoms/images';
import { cube } from '@/images/background';

export default function SellPage() {
  const [amount, setAmount] = useState<string>('0');

  const handleInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value.replace(/[^0-9]/g, '');
    setAmount(value ? Number(value).toLocaleString() : '0');
  };

  const handleButtonClick = (value: number) => {
    const currentAmount = Number(amount.replace(/,/g, '')) || 0;
    const newAmount = currentAmount + value;
    setAmount(newAmount.toLocaleString());
  };

  const handleReset = () => {
    setAmount('0');
  };

  return (
    <div className={'w-full pb-[100px]'}>
      <div className={'my-15'}>
        <div className={'pb-[30px] sm:pb-[60px]'}>
          <div className={'relative w-full h-[200px]'}>
            <Image className={'object-cover'} src={cube} alt={'home-background'} fill quality={100} />

            <div className={'absolute w-max top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2'}>
              <div className={'flex flex-col justify-center items-center gap-2.5'}>
                <div className={'text-gray-100 font-suit-40-800-110 sm:font-suit-60-800-110'}>Service Name</div>

                <div className={'text-gray-100 font-suit-30-400-110'}>구매</div>
              </div>
            </div>
          </div>
        </div>

        <div className={'max-w-[1080px] mx-auto px-3 sm:px-0'}>
          <div className={'w-full pb-5'}>
            <div className={'h-[32px] text-gray-10 font-suit-16-b-130'}>Request for purchase</div>

            <div className={'h-[60px] text-gray-10 font-suit-24-b-130 sm:font-suit-30-700-130 border-b border-gray-50'}>
              구매
            </div>
          </div>

          <div className={'pt-5 pb-10'}>
            <div className={'py-5 flex flex-col gap-5 border rounded-2xl border-gray-0'}>
              <div className={'text-center text-gray-10 font-suit-16-b-130 sm:font-suit-20-b-130'}>
                등록하신 본인 계좌 입금만 허용되며,
                <br className={'sm:hidden'} /> 입금 내역 확인 시<br className={'hidden sm:block'} />
                기재한{' '}
                <span className={'text-orange-orange50'}>
                  은행정보가 예금주가
                  <br className={'sm:hidden'} /> 다를 경우 반환처리
                </span>{' '}
                됩니다.
              </div>

              <div className={'flex flex-col gap-[10px] px-4 sm:px-[60px]'}>
                <div>
                  <div className={'px-1 py-[7px] font-suit-14-m-130 text-gray-40'}>계좌 정보</div>

                  <div className={'h-[38px] flex flex-row gap-1.5 px-[10px] bg-gray-95 rounded-lg items-center'}>
                    <IconLine24SquareInfo className={'text-orange-orange50'} />

                    <div className={'font-suit-13-400-130 text-gray-0'}>
                      계좌정보 변경을 원하실 경우 고객센터로 연락해주세요.
                    </div>
                  </div>
                </div>

                <div className={'flex flex-row sm:flex-row flex-wrap items-center'}>
                  <div
                    className={
                      'flex items-center w-2/5 sm:w-[120px] h-12 sm:h-[56px] p-3 bg-gray-90 border-s border-y border-line-line02 text-gray-0 font-suit-13-b-130'
                    }
                  >
                    은행명
                  </div>
                  <div
                    className={
                      'flex-auto flex items-center h-12 sm:h-[56px] p-3 font-suit-14-r-130 text-gray-0 border border-line-line0'
                    }
                  >
                    KB국민은행
                  </div>
                  <div
                    className={
                      'flex items-center w-2/5 sm:w-[120px] h-12 sm:h-[56px] p-3 bg-gray-90 border-y border-line-line02 text-gray-0 font-suit-13-b-130'
                    }
                  >
                    예금주
                  </div>
                  <div
                    className={
                      'flex-auto flex items-center h-12 sm:h-[56px] p-3 font-suit-14-r-130 text-gray-0 border border-line-line0'
                    }
                  >
                    테라브리스
                  </div>
                  <div
                    className={
                      'flex items-center w-2/5 sm:w-[120px] h-12 sm:h-[56px] p-3 bg-gray-90 border-y border-line-line02 text-gray-0 font-suit-13-b-130'
                    }
                  >
                    계좌번호
                  </div>
                  <div
                    className={
                      'flex-auto flex items-center h-12 sm:h-[56px] p-3 font-suit-14-r-130 text-gray-0 border border-line-line0'
                    }
                  >
                    123456-78-90123
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div>
            <div className={'flex items-center h-[56px] border-b border-gray-0 text-gray-0 font-suit-16-b-130'}>
              구매수량
            </div>

            <div className={'pt-5 pb-10'}>
              <div className={'h-[38px] flex flex-row gap-1.5 px-[10px] bg-gray-95 rounded-lg items-center'}>
                <IconLine24SquareInfo className={'text-orange-orange50'} />

                <div className={'font-suit-13-400-130 text-gray-0'}>
                  최소 구매 수량은 <span className={'text-gray-0 font-suit-15-700-130'}>1,000,000</span>, 최대 구매
                  수량은 <span className={'text-gray-0 font-suit-15-700-130'}>10,000,000</span> 입니다.
                </div>
              </div>

              <div className={'flex gap-2 pt-5 pb-[30px]'}>
                <input
                  type={'text'}
                  className={
                    'flex-1 h-[48px] p-3 text-gray-0 border border-gray-80 rounded-[14px] text-end font-suit-17-m-130'
                  }
                  value={amount}
                  onChange={handleInputChange}
                />

                <button
                  className={
                    'w-2/5 sm:w-[120px] text-gray-100 bg-gray-0 disabled:bg-gray-80 font-suit-17-m-130 rounded-[14px] transition'
                  }
                  onClick={handleReset}
                  disabled={amount === '0'}
                >
                  초기화
                </button>
              </div>

              <div className={'flex flex-col gap-[10px]'}>
                <div className={'grid grid-cols-2 sm:grid-cols-3 gap-[10px]'}>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(5000)}
                  >
                    5,000
                  </button>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(10000)}
                  >
                    10,000
                  </button>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(50000)}
                  >
                    50,000
                  </button>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(100000)}
                  >
                    100,000
                  </button>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(500000)}
                  >
                    500,000
                  </button>
                  <button
                    className={'h-[48px] border border-gray-0 rounded-xl text-gray-0 font-suit-14-b-130'}
                    onClick={() => handleButtonClick(1000000)}
                  >
                    1,000,000
                  </button>
                </div>
              </div>
            </div>
          </div>

          <div className={'pb-5'}>
            <div className={'flex items-center h-[56px] border-b border-gray-0 text-gray-0 font-suit-16-b-130'}>
              구매정보
            </div>

            <div className={'pt-5 pb-10'}>
              <div className={' flex justify-between items-center py-5'}>
                <div className={'text-gray-20 font-suit-18-b-130'}>보유 수량</div>
                <div className={'text-orange-orange50 font-suit-24-750-130'}>0</div>
              </div>

              <div className={' flex justify-between items-center py-5'}>
                <div className={'text-gray-20 font-suit-18-b-130'}>구매 수량</div>
                <div className={'text-orange-orange50 font-suit-24-750-130'}>0</div>
              </div>

              <div className={' flex justify-between items-center py-5'}>
                <div className={'text-gray-20 font-suit-18-b-130'}>남은 수량</div>
                <div className={'text-orange-orange50 font-suit-24-750-130'}>0</div>
              </div>
            </div>

            <button className={'w-full h-[56px] text-gray-100 bg-yellow-50 font-suit-17-b-130 rounded-[14px]'}>
              구매등록
            </button>
          </div>

          <div className={'flex flex-col gap-2.5 rounded-2xl border border-gray-80 p-5'}>
            <div className={'text-gray-30 font-suit-15-m-130'}>✻ 구매/판매 등록 안내</div>

            <ul className={'flex flex-col gap-2.5 list-disc'}>
              <li className={'ms-6 text-gray-20 font-suit-13-m-130'}>
                은행 점검 시간에는 코인전송 외에 구매/판매 신청이 모두 반려됩니다.
              </li>

              <li className={'ms-6 text-red-50 font-suit-13-m-130'}>은행 점검시간 : 23:30 ~ 00:40</li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
}

...
import type { ILoginRequest, ILoginResponse } from './login.types';
import type { OptionsType } from '../_fetch/types';

export async function adminLoginService(data: ILoginRequest, options?: OptionsType) {
...
export async function automaticLoginService(data: ILoginRequest, options?: OptionsType) {
  const response = await fetch<ILoginResponse>('/auth/automatic-login', {
    method: 'POST',
    body: JSON.stringify(data),
    ...options,
  });
